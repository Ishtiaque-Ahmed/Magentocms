...
class NeoTheme_Blog_Adminhtml_Neotheme_Blog_PostController extends NeoTheme_Blog_Controller_Adminhtml_Abstract {
    ...
    private function _getBlogPostData() {
        if (!$this->_blogPostData) {
            $data = array_merge($this->getRequest()->getPost('neotheme_blog'), $this->getRequest()->getPost('meta_data'), $this->getRequest()->getPost('design_data'), $this->getRequest()->getPost('publishing_data'), $this->getRequest()->getPost('content_data')
            );
    ...
    public function saveAction() {
        ...
        if ($data = $this->_getBlogPostData()) {
            ...
            try {
				...
				//print_r($_REQUEST);
                //exit;
                //print_r($this->getRequest()->getParam('banner_image'));
                if (isset($_FILES['banner_image']['name']) && $_FILES['banner_image']['name']) {
                    try {
                        ...
                        $cleaned_image_name = str_replace(' ', '_', $_FILES['banner_image']['name']);
                        //	$dest_banner_img = $path.$cleaned_image_name;
                        //$new_banner_img_name = $uploader->getNewFileName($dest_banner_img);
                        // $uploader->save($path, $_FILES['banner_image']['name']);
                        $uploader->save($path, $cleaned_image_name);
                        $new_banner_img_name = 'blog/' . $uploader->getUploadedFileName();
                        $data['banner_image'] = $new_banner_img_name;
//                        var_dump($data['banner_image']);
//                        exit();
                        //$model->setBannerImage($_FILES['banner_image']['name']);
                    } catch (Exception $e) {
...
//                        $image = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'blog' . DS . $model->getBannerImage();
...
//                if (is_array($data['banner_image']) && count($data['banner_image']) > 0) {
...
//                }
                Mage::log($model->getData(), null, 'system.log');
                $model->save();
